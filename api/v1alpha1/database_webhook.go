...
func (r *Database) ValidateUpdate(old runtime.Object) error {
	...
	// if it's just a status change let it go through
	if !r.Spec.ForceUpdate {
		if r.Status.Status == "" {
			return nil
		}
		databaselog.Info("forceUpdate new status", "new-status", r.Status.Status)
		databaselog.Info("forceUpdate current status", "current-status", curr.Status.Status)
		if r.Status.Status != curr.Status.Status || r.Status.Status == DatabaseConditionCreated {
			return nil
		}
	}
	allErrs = append(allErrs, field.Invalid(field.NewPath("spec").Child("forceUpdate"), r.Spec.Name, "cannot modify database with forceUpdate is `false`"))
	return apierrors.NewInvalid(
		schema.GroupKind{Group: "sqlmi.arc-sql-mi.microsoft.io", Kind: "Database"},
		r.Name, allErrs)
}
